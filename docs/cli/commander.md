...
可以自定义一些命令行指令，在输入自定义的命令行的时候，会去执行相应的操作。
...
program
  .name('我的脚手架')
  .description('我的 JavaScript 脚手架工具')
  .version('0.8.0');

// 定义一个名为'split'的命令
program.command('split')
  // 设置命令描述
  .description('把字符串变成数组')
  // 设置命令参数
  .argument('<string>', 'string to split')
  // 设置命令选项
  .option('--first', "如果选项中有'--first'，则只显示第一个子字符串，否则显示所有子字符串")
  .option('-s, --separator <char>', '输入一个分隔符，将字符串分割转为数组输出，\n例如输入 split -s / a/b/c \n或 split --separator=/ a/b/c', ',')
  // 设置命令动作
  .action((str, options) => {
    console.log(str, options);
    // 如果选项中有'first'，则只显示第一个子字符串，否则显示所有子字符串
    const limit = options.first ? 1 : undefined;
    // 使用分隔符将字符串分割成子字符串，并输出结果
...
Options:
  --first                 如果选项中有'--first'，则只显示第一个子字符串，否则显示所有子 字符串
  -s, --separator <char>  输入一个分隔符，将字符串分割转为数组输出，
                          例如输入 split -s / a/b/c
                          或 split --separator=/ a/b/c (default: ",")
  -h, --help              display help for command

$ node string-util.js split --separator=/ a/b/c
$ node string-util.js split --separator / a/b/c
$ node string-util.js split -s=/ a/b/c 
$ node string-util.js split -s / a/b/c 
a/b/c { separator: '/' }
[ 'a', 'b', 'c' ]

$ node string-util.js split --separator=/ a/b/c --first
a/b/c { separator: '/', first: true }
[ 'a' ]
```
...
// 添加选项
program
  .option('-d, --debug', 'output extra debugging') // 添加debug选项，输出额外的调试信息
  .option('-s, --small', 'small pizza size') // 添加small选项，表示小尺寸的披萨
  .option('-p, --pizza-type <type>', 'flavour of pizza'); // 添加pizza-type选项，表示披萨的口味
...
// 如果有small选项，则输出小尺寸的披萨
if (options.small) console.log('- small pizza size');
// 如果有pizza-type选项，则输出披萨的口味
...
PS D:\Code\blog\test> node index.js pizza-options -p
error: option '-p, --pizza-type <type>' argument missing

PS D:\Code\blog\test> node index.js pizza-options -d -s -p vegetarian
...
PS D:\Code\blog\test> node index.js -c blue
{ cheese: 'blue' }
blue

PS D:\Code\blog\test> node index.js -c yellow
{ cheese: 'yellow' }
yellow
```
...
program.version('0.0.1', '-v, -V, --vers, --version');
...
PS D:\Code\blog\test> node index.js -V
0.0.1
PS D:\Code\blog\test> node index.js -version
0.0.1
```
