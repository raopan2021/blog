# React Native

> 跨平台移动端开发框架，使用 JavaScript 和 React 构建原生应用

## 📚 快速导航

### 基础入门
- [简介](./basic/introduce)
- [环境搭建 - iOS](./basic/env-ios)
- [环境搭建 - Android](./basic/env-android)
- [项目结构](./basic/structure)

### 内置组件
- [View](./components/view)
- [Text](./components/text)
- [Button](./components/button)
- [Image](./components/image)
- [ImageBackground](./components/imageBackground)
- [TextInput](./components/textInput)
- [Switch](./components/switch)
- [Modal](./components/modal)
- [ScrollView](./components/scrollView)
- [KeyboardAvoidingView](./components/keyboardAvoidingView)

### UI 与布局
- [样式](./ui/style)
- [布局](./ui/layout)
- [事件处理](./ui/event)
- [动画](./ui/animate)
- [图片](./ui/image)
- [网络请求](./ui/network)

### 原理与架构
- [架构原理](./principle/architecture)
- [渲染流程](./principle/render)
- [通信机制](./principle/communication)

### 构建与发布
- [发布流程](./build/publish)
- [iOS 发布](./build/publish-ios)
- [Android 发布](./build/publish-android)
