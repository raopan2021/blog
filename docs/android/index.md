...
📦 第一阶段：入门（⭐）
   ...
   01-项目概览 → 02-环境搭建 → 03-项目结构 → 08-Compose基础

📦 第二阶段：进阶（⭐⭐）
   掌握核心技能，独立开发功能模块
   
   04-数据模型 → 05-持久化 → 06-状态管理 → 07-网络请求

📦 第三阶段：实战（⭐⭐⭐）
   综合运用，完整实现一个功能模块
   
   09-Compose进阶 → 10-账户管理 → 11-设置页面 → 12-日志系统 → 13-构建发布
```
...
| 章节 | 内容 | 难度 |
|------|------|------|
| [项目概览](./01-项目概览) | 项目介绍、技术栈对比表、与前端概念的对应关系 | ⭐ |
| [环境搭建](./02-环境搭建) | JDK、Android SDK、Gradle 安装配置 | ⭐ |
| [项目结构](./03-项目结构) | 目录结构、构建配置、文件组织 | ⭐ |
| [Compose 基础](./08-Compose基础) | @Composable、State、布局组件、列表渲染 | ⭐⭐ |
...
| [持久化](./05-持久化) | DataStore + Repository 模式、Flow 响应式数据流 | ⭐⭐ |
| [状态管理](./06-状态管理) | ViewModel + StateFlow + Coroutines | ⭐⭐ |
| [网络请求](./07-网络请求) | HttpURLConnection、Coroutines、重试机制、JSON解析 | ⭐⭐⭐ |
