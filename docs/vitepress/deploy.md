...
默认情况下，我们假设站点将部署在域名 （`/`）的根路径上。如果你的网站将在子路径中提供服务，例如 `https://mywebsite.com/blog/`，则需要在 VitePress 配置中将 [`base`](../vitepress/site-config#base)选项设置为 `'/blog/'`。
...
注意：该 `_headers` 文件应放置在[public 目录](/vitepress/asset-handling#the-public-directory)中（在我们的例子中是 `docs/public/_headers`），以便将其逐字复制到输出目录。
